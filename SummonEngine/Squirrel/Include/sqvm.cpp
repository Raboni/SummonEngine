...
bool SQVM::ToString(const SQObjectPtr &o,SQObjectPtr &res)
{
    ...
    default:
		const size_t count = sq_rsl((sizeof(void*) * 2) + NUMBER_MAX_CHAR);
        scsprintf(_sp(sq_rsl((sizeof(void*)*2)+NUMBER_MAX_CHAR)), count, count,_SC("(%s : 0x%p)"),GetTypeName(o),(void*)_rawval(o));
    }
    ...
    //
    {
        for(;;)
        {
            ...
            case _OP_CALL: {
                    ...
                    case OT_INSTANCE:
					{
                        SQObjectPtr closurePtr;
                        if(_delegable(clo)->_delegate && _delegable(clo)->GetMetaMethod(this,MT_CALL, closurePtr)) {
                            Push(clo);
                            for (SQInteger i = 0; i < arg3; i++) Push(STK(arg2 + i));
                            if(!CallMetaMethod(closurePtr, MT_CALL, arg3+1, clo)) SQ_THROW();
                            if(sarg0 != -1) {
                                STK(arg0) = clo;
                            }
                            break;
...
//      SQInteger n = 0;
        ...
        while( ci ) {
            if(ci->_etraps > 0) {
                SQExceptionTrap &etr = _etraps.top();
                ci->_ip = etr._ip;
                _top = etr._stacksize;
                _stackbase = etr._stackbase;
                _stack._vals[_stackbase + etr._extarget] = currerror;
...
void SQVM::CallDebugHook(SQInteger type,SQInteger forcedline)
{
    ...
    if(_debughook_native) {
        ...
        SQObjectPtr temp_regi;
        SQInteger nparams=5;
        Push(_roottable); Push(type); Push(func->_sourcename); Push(forcedline?forcedline:func->GetLine(ci->_ip)); Push(func->_name);
        Call(_debughook_closure,nparams,_top-nparams,temp_regi,SQFalse);
        Pop(nparams);
    }
    _debughook = true;
}
...
cloned_mt:
        ...
        if(_delegable(newobj)->_delegate && _delegable(newobj)->GetMetaMethod(this,MT_CLONED,closure)) {
            Push(newobj);
            Push(self);
            if(!CallMetaMethod(closure,MT_CLONED,2,temp_regi))
                return false;
        }
        }
        target = newobj;
