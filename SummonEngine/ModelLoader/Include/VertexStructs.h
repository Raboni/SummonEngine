...
{
public:
	SVertexCollection(int aDataSize)
	{
		myData.reserve(aDataSize);
	}
	void PushVec4(FBXLoader::Vector4f aPos)
	{
		myData.resize(myData.size() + 4);
		memcpy(&myData[myData.size() - 4], &aPos, sizeof(FBXLoader::Vector4f));
	}
	void PushVec2(FBXLoader::Vector2f aPos)
	{
		myData.resize(myData.size() + 2);
		memcpy(&myData[myData.size() - 2], &aPos, sizeof(FBXLoader::Vector2f));
	}
